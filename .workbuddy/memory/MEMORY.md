...
- 📏 云端库实测（2026-09-25 抓 CDN）：`cy-chat@main/Word/word.json` = 3 组 1521 条
  （**只有「自定义回复」有内容，另两组 0 条**；空正文 0 条）；
  `cy-chat@main/Meme/meme.json` = 4 组 78 条，`url` 一律 `Meme/images/...` 相对路径。
  两边**云端内部都没有重复项** → 重复只发生在本地侧

## ⛔ 列表项「选中态」不能拿渲染下标当身份（v1.15.3 教训）
- 云端库复选框的 id 原来是**渲染时的数组下标**（字卡 `${cat}::${ii}`、表情 `csk_${i}`），
  导入时却拿这个下标回**原始数组**取值 → 渲染前只要有过一次 `filter`，两边就不是同一个数组
- 字卡：渲染会滤掉空正文项 → **空项之后的行整体错位**（云端 word.json 目前无空项 → 潜在）
- 表情：`renderCloudStickerGrid` 收的是**搜索过滤后的子集**，导入按全量 `_cskData` 取
  → **一搜索就导错**（勾这张、进来另一张）
- ✅ 修法：下标一律回**原始数组**取（`.map((item,oi)=>({item,oi}))` 之后再 filter；
  或 `(window._cskData||stks).indexOf(s)`），**筛选只影响显示、不影响身份**
- 🔎 通用判据：**凡"渲染的是子集、却按位置回查原数组"的地方，都是同一个 bug 的形状**
- ⚠ 残留（未修、影响极小）：`selectAllCloudCards` 会把空正文项的 id 也选上，
  按钮计数比实际多 1（真正导入时 `if(text)` 会滤掉，结果不受影响）

## ⛔ 已发布的版本号不能复用
- 本轮修复曾被**直接追加进 v1.15.2 的段落**，而 v1.15.2 早已发布（commit `1b43acc`）
  → 同一个版本号对应两份不同代码，手机上**没法靠版本号判断修复到没到**（那是唯一判据）
- ✅ 规则：**任何在已发布版本之上的改动都另起一号**；CHANGELOG 的旧段落保持"当初发布时的样子"
