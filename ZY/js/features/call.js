...
<div id="call-mini-pill">
  ...
  <button class="call-mini-hangup" id="call-mini-hangup">${SVG_HU}</button>
    ...
    function bindEvents() {
        ...
        // ★ 使用 pointerdown + stopPropagation 阻止拖拽 header 捕获指针，解决 click 失效问题
        document.getElementById('call-minimize-btn')?.addEventListener('pointerdown', function(e) {
            e.stopPropagation();
            minimizeWindow();
        });
        // ★ 使用 pointerup 代替 click，避免 pointerdown.preventDefault 导致的 click 事件丢失
        document.getElementById('call-mini-pill')?.addEventListener('pointerup', function(e) {
            if (e.target.closest('.call-mini-hangup')) return;
        ...
        document.getElementById('call-size-preset-toggle')?.addEventListener('pointerdown', function(e) {
            e.stopPropagation();
            const p = document.getElementById('call-size-presets');
            if (p) { p.classList.contains('open') ? p.classList.remove('open') : openSizePresets(); }
        });
